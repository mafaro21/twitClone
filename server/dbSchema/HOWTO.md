...
These are **Data Models** as well as **Validation rules** set up for each ``Collection`` for your MongoDb ``Database``. They enforce data structure and integrity at **Database Level** during inserting and updating, *regardless of the application* used to interact with the database. They are called JSON Schema. Learn more about them here: [https://docs.mongodb.com/manual/core/schema-validation/]
...
2. Now open `users.txt` file contained here, **SelectAll** and **Copy** (the *content* not THE FILE itself!!!).
3. Go back to MongoDB Compass > `yourDatabaseName` > `users` Collection > click **Validation** tab.
...
- You can always **Update** the rules using MongoDB Compass (or CLI) whenver or however you wish.